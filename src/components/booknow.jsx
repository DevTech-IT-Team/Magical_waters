...
const BookNow = () => {
  ...
  const onSubmit = async (event) => {
    ...
    const accessKey = import.meta.env.VITE_WEB3FORMS_ACCESS_KEY || "b57a7038-03a2-4e48-89e3-62a6b2f48eff";

    if (!accessKey || accessKey === "YOUR_ACCESS_KEY_HERE") {
      setStatus("error");
      setResult("Missing API Key. Please add your Web3Forms Access Key to your configuration.");
      return;
    }
    ...
    // Convert FormData to JSON object (Web3Forms recommended approach for React)
    ...
    try {
      ...
      if (response.status === 200 || data.success) {
        ...
        setStatus("error");
        setResult(data.message || "Submission failed. Please check your Access Key settings.");
      }
    } catch (error) {
      console.error("Submission Network Exception:", error);
      setStatus("error");
      setResult("Network error. Please check your console logs or connection.");
    }
  };
  ...
  return (
    <section id="booknow" className="min-h-screen bg-slate-50 flex items-center justify-center p-6 py-24">
      <div className="max-w-6xl w-full grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
        ...
        <div className="space-y-8">
          <div>
            <h1 className="text-4xl lg:text-5xl font-serif italic text-blue-900">
              Reserve a <span className="text-blue-600">Calm Session</span>
            </h1>
            <p className="mt-4 text-slate-500 text-lg leading-relaxed max-w-md">
              Choose a day that feels right, and we will connect to confirm your session with ease.
            </p>
          </div>
          ...
          <form onSubmit={onSubmit} className="bg-white p-8 rounded-[2rem] border border-blue-100 shadow-2xl shadow-blue-900/5 relative overflow-hidden">
            ...
            {status === "success" && (
              <div className="absolute inset-0 bg-white/95 backdrop-blur-sm z-20 flex flex-col items-center justify-center text-center p-6 animate-in fade-in duration-500">
                ...
                <button type="button" onClick={() => setStatus("idle")} className="px-6 py-2 bg-blue-600 text-white rounded-full text-sm font-semibold">Send Another</button>
              </div>
            )}

            {/* Optimized Honeypot field for Web3Forms spam tracking */}
        ...
        {/* Right Side: Calming Detail Image */}
        <div className="relative flex justify-center items-center">
          <div className="w-full max-w-md aspect-square overflow-hidden rounded-[2rem] border border-blue-100 shadow-2xl shadow-blue-200/20">
            <img
              src="/heartbottle.jpg"
              alt="Heart glass bottle for calm hydration"
